...
public abstract class GeneradorResumenComprasUsuario {
	...
	public static String generarResumen(Usuario usuario) {
		...
		if (usuario.getPropuestasContratadas().size() == 0) {
			...
			mensaje += "Total a pagar: " + totalApagar + " créditos galácticos (saldo disponible: "
					+ usuario.getPresupuestoDisponible() + " créditos galácticos).\n";
			mensaje += "Tiempo utilizado " + ModificadorFormatoHora.obtenerHoraConFormato(tiempoUtilizado)
					+ " (tiempo disponible "
